...
for fq1 in os.path.join (wd, '*_R1.fastq.gz'):
    ...
    for item in file:
        ...
        compressed_variants = os.path.join (wd, 'results', 'vcf', f'{base}' + '_final_variants.vcf.gz')
        indels = os.path.join (wd, 'results', 'vcf', 'indels', f'{base}' + '_indels.vcf') 
        comprssed_indels = os.path.join (wd, 'results', 'vcf', 'indels', f'{base}' + '_indels.vcf.gz') # same name as the uncompressed
        snvs = os.path.join (wd, 'results', 'vcf', 'snvs', f'{base}' + '_snvs.vcf')
        comprssed_snvs = os.path.join (wd, 'results', 'vcf', 'snvs', f'{base}' + '_snvs.vcf.gz')
        ...
        for command in (f"bwa mem {ref_genome_file} {fq1} {fq2} | samtools sort -o {sorted_bam}",
                        ...
                        f"bgzip -c {final_variants} > {compressed_variants}",
                        #f"gzip {final_variants}",
                        f"bcftools index {compressed_variants}",
        ...
        for command in ("cat combined_indels_variants.csv combined_snv_variants.csv > dndscv.csv",
                        "sed 's/,/\t/g' dndscv.csv > file.txt",
                        "column -t file.txt > dndscv.txt"):
              call(command, shell=True)  
